"""
Commandes admin (Telegram) pour piloter les enrollments multi-KYC :
  /enrollments  — liste tous les enrollments (statut, email, état)
  /pending      — KYC en attente de validation (+ account_id à donner à Interlace)
  /available    — cartes créées NON réclamées (+ liens à transmettre)
  /finalize <account_id> — crée la carte après validation Interlace
  /adminhelp    — aide
...
async def cmd_enrollments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows[:_MAX]:
        ...
        state = "🟢 réclamé" if r.get("USER_ID") else ("💳 carte prête" if r.get("card_id") else "⏳")
...
async def cmd_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    lines = [f"⏳ KYC en attente ({len(rows)}) — account_id à valider chez Interlace :"]
    for r in rows[:_MAX]:
        email, _ = _email_name(r)
...
async def cmd_available(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not av:
        ...
        return
    lines = [f"🎟 Cartes prêtes NON réclamées ({len(av)}) — liens à transmettre :"]
    for r in av[:_MAX]:
        email, _ = _email_name(r)
...
async def cmd_finalize(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage : /finalize <account_id>")
        return
    account_id = context.args[0].strip()
    await update.message.reply_text(f"⏳ Finalisation de {account_id}…")
    try:
        from services.interlace_kyc import complete_after_kyc_passed
        res = await complete_after_kyc_passed(account_id)
    except Exception as e:
        logger.error(f"[admin] finalize {account_id}: {e}")
        await update.message.reply_text(f"❌ Erreur : {e}")
        return
    if res.get("success"):
        link = res.get("link") or "—"
        await update.message.reply_text(f"✅ Carte créée.\nLien : {link}",
                                        disable_web_page_preview=True)
    else:
        await update.message.reply_text(f"❌ Échec : {res.get('message', 'inconnu')}")


async def cmd_adminhelp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🛠 Commandes admin :\n"
        "/enrollments — tous les enrollments + état\n"
        "/pending — KYC en attente (account_id à valider chez Interlace)\n"
        "/available — cartes prêtes non réclamées + liens\n"
        "/finalize <account_id> — créer la carte après validation\n"
